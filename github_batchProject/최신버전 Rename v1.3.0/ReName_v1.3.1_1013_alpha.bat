...
rem ### 현재버전정보		:	v1.3.1 v221013_alpha
...
rem ########## 안내문 ############
echo.
echo 	##1.3.1 v221012_alpha 업데이트 내용##
echo.
echo 	## 1. 1.3.0 오늘날짜 기본입력 및 수정 가능
echo 	## 2. 재실행 선택 추가
echo 	## 3. 코드 최적화
echo.
rem pause
...
rem ##########선택 옵션##########
echo.
rem choice /c 12 /n /m "	1.이름재설정 2.처음으로 돌아가기	0.취소"
echo ------------------------------------------------------
echo    1. 이름 재설정 완료, 종료    2. 재설정 후 재실행
echo.
echo    3. 처음으로 돌아가기         4. 재설정하지 않고 종료
echo -------------------------------------------------------
echo.
set /p last=Choice : 
rem if %errorlevel% equ 0 goto quit
rem if %errorlevel% equ 1 goto rename
rem if %errorlevel% equ 2 goto OnDirectory

if %last% equ 1 goto rename
if %last% equ 2 goto rename
if %last% equ 3 goto OnDirectory
if %last% equ 0 goto quit
...
if %last% equ 2 (
	echo 재실행을 선택했습니다
	echo 아무키나 누르면 재실행창으로 넘어갑니다
	echo 이전 경로와 입력된 값이 유지됩니다.
	pause
	cls
	goto Notice
)
